...
class ExecutionEngine 
{
 public:
  ...
  virtual std::vector<const Tensor*> forward(const std::vector<VariableIndex>& node_list);
...
class BatchedExecutionEngine : public ExecutionEngine 
{
 ...
 private:
  const Tensor& incremental_forward_no_update(VariableIndex upto, int autobatch_strategy);
  void combine_tensors(const std::vector<VariableIndex>& batch_ids, int aid, Tensor &tout);
  void accumulate_tensors(const Tensor& tin, const std::vector<VariableIndex>& batch_ids, int ai);
