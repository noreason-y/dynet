...
void Node::forward(const std::vector<const Tensor*>& xs, Tensor& fx) const 
{
  ...
  {
    DYNET_RUNTIME_ERR("Node " << this->as_dummy_string() 
        << " does not support batching but got fed batched tensor");
  }
}
...
void Node::autobatch_reshape_concatonly(
...
VariableIndex ComputationGraph::add_input(
    const Dim& d, const vector<unsigned int>& ids, 
...
VariableIndex ComputationGraph::add_lookup(
    LookupParameter p, 
...
void ComputationGraph::print_graphviz() const 
{
  ...
  for (auto node : nodes) 
  {
    ...
    cerr << "  N" << nc << " [label=\"v" << nc << " = " << node->as_string(var_names);
    if (profiling_flag)
    {
      cerr << " (MEM: ";
      ...
      {
        cerr << std::setprecision(4) << std::setw(11) 
             << (node->aux_storage_size() + 2*node->dim.size() * sizeof(real)) / 1024.0
             << " KiB)";
      }
    }
    cerr << "\"];\n";
  ...
  if (profiling_flag>1) 
  {
    ...
    {
      std::cerr << std::setprecision(4) << std::setw(11) << (item.first/1024.0) << " KiB\t" 
                << (100.0*(double)item.first/total_memory) << "%\t" << "called " 
                << count_map[item.second] << "x\t" << item.second << std::endl;
    }
